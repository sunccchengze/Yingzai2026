...
def build_beak(r: Refs, m_beak, m_beak_d, m_mouth, m_tongue, coll):
    ...
    for k in range(n_len):
        ...
        # 坑：菱形最宽的那一圈在 root_y=-0.20，而头部前表面在 y≈-0.44，
        # 即「最宽处埋在头里面」，正面只能看到已经收窄的前段 → 渲成扁帽子。
        # 因此收敛必须**等喙钻出头部之后**才开始：t<t_out 时保持满宽。
        t_out = 0.40
        f = 0.0 if t <= t_out else ((t - t_out) / (1.0 - t_out)) ** 1.35
        ring = []
        for (px, pz) in face_pts:
            # 朝喙尖点收敛
            nx = px * (1 - f) + tip_pt[0] * f
            nz = pz * (1 - f) + tip_pt[1] * f
            # 末端再补一点下勾（鹰钩），但幅度远小于菱形高度
            nz -= r.L(16) * (f ** 2.2)
...
def build_feet(r: Refs, m_foot, coll):
    ...
    z0 = r.Z(606)          # 脚底（贴地）
    ztop = r.Z(545)        # 脚背顶（抬高，塞进身体下缘）
    h = ztop - z0
    toe_len = r.L(112)     # 单根趾长度（侧视单趾实测 ~112px）
    toe_r = r.L(20.5)      # 单根趾半径（圆头趾）
