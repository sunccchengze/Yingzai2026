...
# ════════════════════════════════════════════════════════════ 各部件
# ── 第24轮：大头娃娃比例 ──
# 参考头占全高 60.1%，模型只有 52.6%。头整体放大这个系数，
# **所有依附在头上的部件（眼/冠羽/披肩）必须同步缩放**，
# 否则头一大，眼睛就显小、冠羽被吞、披肩被撑成直边。
# 坑：首版 HEAD_S 等比放大 1.14 —— 头占全高 52.6%→55.9%(好)，
# 但**头宽/身宽从 0.870 冲到 0.992**（参考 0.858），头变得和身体一样宽，
# 披肩被撑成两条直边、失去扇贝。
# 参考的大头是**高而不宽**：头高要涨、头宽几乎不动。故拆成两个系数。
# 坑：1.16 让头顶冲到行 y21，而参考 y60 处白宽仅 83px、y70 才 110px ——
# **参考的头顶真正开始在 y70 左右，上面全是冠羽**。头长过了，
# 正面 y60 多出 +106px，整个脑袋方了。降到 1.07。
HEAD_S = 1.07      # 高度方向（决定"头占全高"）
HEAD_SW = 1.005    # 宽度方向（几乎不动，保住 头宽/身宽 ≈0.86）
...
def build_head(r: Refs, m_white, coll):
    ...
    # 第21轮修正：上一轮把头前移 7px 提了 side 分数, 但 evaluated mesh 量出
    # 头前缘落到侧视 x=51, 而参考头前缘在 x≈74(y210 行), 喙根在 x=108 ——
    # **头把整个喙根吞了**, 渲染里喙只露出前端一小截薄片, 且喙/头之间
    # 出现 36~69px 的内部空洞(侧面 y238..262 每行缺 34~74px)。
    # 头改为后退 6px, 让头前缘回到 x≈70, 喙自然从脸部伸出。
    # ── 第22轮（以造型为准，不再唯 IoU）：
    # evaluated mesh 逐行对比「模型头前缘 vs 参考 white 前缘」（侧视 x，越小越靠前）：
    #   y180 −13 / y200 −18 / y220 −15 / y240 −20 / y250 −26 / y260 −21 / y280 −12
    # 即整个头在 Y 向**前伸了 11~26px**，喙区(y240..260)最严重。
    # 脸太鼓 → 把喙根(侧视 x107)整段吞进去，渲染只剩脸外一截薄弯钩。
    # 参考图里 y252 行 white 前缘 x112、黄喙根 x107 —— 喙根本就紧贴脸，
    # 是我的脸冲到了 x85 才盖住它。
    #
    # 修法：① 取消 +7px 前移（那是唯 IoU 时代的产物，正是它把脸推向喙）；
    #       ② Y 半径的 1.032 放大改为 0.94 收缩，让前缘回到实测位置。
    # X 向不动（正面/背面宽度目前是准的）。
    # ── 第24轮：**回到整体比例**（此前 20+ 轮全在抠零件，忽略了最根本的事）──
    # 参考图是**大头娃娃**：头占全高 60.1%、头宽/身宽 0.858 —— 这正是萌感来源。
    # 实测我的模型：头宽/身宽 0.870（准），但**头只占全高 52.6%**，矮了 7.5 个百分点。
    # 之所以 20 多轮没发现：IoU 按包围盒归一，**对整体比例完全不敏感** ——
    # 头小一整档，剪影重合度照样能到 0.95。这是方法论上的盲区。
    #
    # 修法：头整体放大 HEAD_S，且**以头的底部为锚点向上长**
    # （直接缩放会让头往下陷进身体，或整体飘起来）。
    z_bot_head = min(z for (z, _ax, _ay, _cx, _cy) in rings)
    rings = [(z_bot_head + (z - z_bot_head) * HEAD_S,
              (ax + pad) * 1.028 * HEAD_SW, (ay + pad) * 0.94 * HEAD_SW, cx, cy)
             for (z, ax, ay, cx, cy) in rings]
...
def build_head_fringe(r: Refs, m_white, coll):
    ...
    # 坑（第23轮四版）：正面头部两侧描边**断续发虚** —— 披肩在 y292 处
    # X 半径 164px、头 167px，两个曲面只差 3px 近乎重合，
    # Freestyle 在这种紧贴处判定混乱，轮廓线时有时无。
    # 上环再收窄，让披肩上段与头面之间留出足够间隙。
    ax_top, ay_top = r.L(128.0) * HEAD_SW, r.L(96.0) * HEAD_SW  # 随头(宽度)
...
def build_crest(r: Refs, m_white, coll):
    ...
    # 头放大后顶到行 y21（原 y60），根部必须下探到 y21 以下才咬得住，
    # 且 y30..60 段要加粗，补上参考 y40 处 93~107px 的宽度（原来只有 53~76px）。
    # 62/70 加粗过头：冠羽融进头顶变成方脑袋，呆毛几乎消失。
    # 只需保证根部咬住新头顶(y21)，粗细回到原值。
    spine = (
        (76, 206, 222, 40, 46),   # 根部：埋进头内部
        (60, 210, 224, 33, 41),
        (48, 214, 228, 28, 36),
        (38, 226, 228, 25, 35),
        (32, 234, 222, 24, 34),
    ...
    n = 24
    rings = []
    # 坑（第24轮）：一度把冠羽也乘 HEAD_S，结果整撮被抬到行 y-44
    # **冲出画布顶部**（参考冠羽只到 y3），侧面 y40..60 还塌出一个凹陷。
    # 冠羽的**绝对位置由参考决定，不该跟着头缩放**；
    # 头长高后需要调整的只是「根部埋多深」—— 让根接住新的头顶即可。
...
def build_beak(r: Refs, m_beak, m_beak_d, m_mouth, m_tongue, coll):
    ...
    # ── 第21轮重写：肉眼验收发现侧面喙是**细长下垂的管子**（像鹬鸟），
    # 而参考是**粗短的三角钩**，且喙与头之间还裂开一条 36~69px 的内部空洞
    # （侧面 y238..262 每行缺 34~74px，但左右端点 Δ 只有 +1~+11 ——
    #  缺口不在轮廓端点而在**内部**，这是穿帮的破洞，IoU 却看不出严重性）。
    #
    # 重新按侧视 yellow 掩膜逐行实测（y<340 排除脚）：
    #   y=192 宽  6   y=210 宽 34   y=228 宽 56   y=246 宽 91
    #   y=198 宽 21   y=216 宽 43   y=234 宽 65   y=252 宽101
    #   y=204 宽 25   y=222 宽 52   y=240 宽 74   y=258 宽102(x 6..108)
    # 宽度**单调递增**：上喙是一个「尖在上(y192)、根在下(y258)」的下张三角，
    # 整块紧贴脸部，根本不是朝前收敛的长管。原 profile 的 (68,196,259)
    # (56,186,256) 把上缘顶到 y186 且中段最高、两端低，正是长管的成因。
    #
    # 改：以侧视 x 为参数，上缘 y 从喙根(x=104,y≈250) 单调升到喙尖(x=6,y≈200),
    # 下缘 y 稳定在 258..268(含描边)，让剖面从「窄根」张成「宽尖」的钩形。
    # 第21轮二修：evaluated mesh 量出喙根 Z 高只有 15px、宽高比 7.95 ——
    # 喙被压成一片**薄板**，侧视自然只剩一道细边。参考侧视喙根高约 60px。
    # 上缘沿用实测(尖在上 y192 → 根在下 y252 单调下降)，
    # 但**下缘一律压到 y262..276**，保证每个截面都有 25~65px 的实厚度。
    # ── 第22轮：按侧视 yellow **逐列**实测的真实脊线重建（上一轮按行测，
    # 只拿到宽度包络，丢了上缘的拱形，做出来的喙朝左下斜插）。
    # 逐列实测（x 越小越靠前/喙尖）：
    #   x108 y254..259(厚 5)   x 60 y193..307(厚114/含下喙)
    #   x102 y248..262(厚14)   x 54 y194..307
    #   x 96 y245..267(厚22)   x 48 y197..307
    #   x 90 y243..278(厚35)   x 42 y200..251(厚51)
    #   x 84 y239..289(厚50)   x 36 y208..250(厚42)
    #   x 78 y233..296(厚63)   x 30 y214..250(厚36)
    #   x 72 y223..301(厚78)   x 24 y221..251(厚30)
    #   x 66 y196..305(厚109)  x 18 y221..254(厚33)
    #                          x 12 y231..257(厚26)
    #                          x  6 y249..263(厚14)
    # 上缘是一条**先升后降的拱**：喙根 y254 → x54~60 隆到最高 y193 → 喙尖回落 y249。
    # 这就是鹰喙的隆起。x48..66 段厚度 110+ 是因为把下喙/口腔也算进去了，
    # 上喙自身下缘取 y250..260 一线（见 x42 的 y251、x72 的 y223..301 中的上半）。
    # ── 第22轮三修：上缘经 evaluated mesh 验证已贴合参考(Δ±7px)，
    # 但渲染仍是"细钩"—— 因为**厚度不够**。参考逐列实测的上喙本体厚度：
    #   x  6 厚14   x 30 厚36   x 54 厚61(最厚)   x 78 厚26
    #   x 14 厚22   x 38 厚42   x 62 厚19*        x 86 厚17
    #   x 22 厚29   x 46 厚54   x 70 厚42         x102 厚14
    #  (*x62 那列被张开的嘴切断，实际应与邻列相当)
    # 我原来的截面只有 30~35px，不到参考最厚处的六成，自然显得又薄又尖。
    # 下缘 = 上缘 + 实测厚度，让喙在 x38..70 段饱满隆起。
    beak_profile = [
    ...
    pz_lo = min(pz for (_px, pz) in face_pts)
    pz_hi = max(pz for (_px, pz) in face_pts)
    rings = []
    for k, (sx, y_up, y_dn) in enumerate(beak_profile):
        ...
        # 第21轮：原式让喙尖只剩 26% 宽，渲染出来是**贴在脸侧的薄片**。
        # 正面实测喙根 y268 宽 133px(半宽 66)、侧视同高 102px —— 喙是又宽又厚
        # 的实体，喙尖(正面 y214 宽 6px)虽收窄但整体应保持三角锥的体积感。
        # 收窄曲线放缓（指数 0.85->1.6 使前段保持宽厚，尾段才快速收）。
        wf = (1.0 - t) ** 1.6 * 0.62 + 0.38
        ring = []
        for (px, pz) in face_pts:
            ...
            # 坑（第22轮，改了三次 beak_profile 都不生效才定位到）：
            # 这里原本用 z_top=r.Z(217) / z_bot=r.Z(333) 这对**固定老常量**归一，
            # 跨度 116 行；而 face_pts 的 pz 实际只落在 y214..272（58 行）。
            # 于是 u 被压在 0.53..1.0 的窄区间里，
            # (u-0.5)*2*half_z 只能取到 half_z 的一小部分 ——
            # **beak_profile 里写的厚度被这个映射吃掉了大半**，
            # 所以无论把 y_dn 压多低、厚度写到 61px，渲染出来始终是薄细钩。
            # 改用 face_pts 自身的真实 z 范围归一，让 u 铺满 0..1。
            span = max(1e-6, pz_hi - pz_lo)
            u = (pz - pz_lo) / span            # 0..1 在原菱形里的相对高度
    ...
    # 第22轮：evaluated mesh 量出下颌实际落在侧视 x63..92 / 行 y269..332，
    # 而参考下喙黄是 x44..100 / y262..307 —— **向下多伸了 25px、前端少了 19px**，
    # 导致侧视里下喙被身体和颈羽埋掉，只在脸下方露出一小片红黄，
    # 上喙因此显得孤零零地斜插在外。收浅 + 前伸让它露出来。
    z_lip = r.Z(266)      # 唇线：与上喙下沿同高，接住它
    z_chin = r.Z(309)     # 下颌最低（对齐参考 y307）
    # 第22轮：evaluated mesh 量出下颌侧视只占 x55..84（29px），
    # 而参考下喙是 x44..100（56px）—— 只有一半长，
    # 前端 16px 露在脸外(头前缘 x71)、后半截埋进脸里，
    # 渲染出来就是脸下方一块**脱开的红黄碎片**，而不是张开的嘴。
    # 加大半宽让它横跨整个 x44..100，两端分别咬住喙尖侧与脸颊。
    ...
    objs.append(disc("喙_下颌", m_beak_d, mouth_w, hh_mouth,
                     zc_mouth, y_face + r.L(2), r.L(15), squash_top=0.06))
    # ② 口腔（红）：内缩一圈，露出四周一道黄色厚唇
    objs.append(disc("口腔", m_mouth, mouth_w * 0.80, hh_mouth * 0.80,
                     zc_mouth - r.L(4), y_face - r.L(6), r.L(9),
                     squash_top=0.06))
...
def build_eyes(r: Refs, m_dark, m_white, coll):
    ...
    # 头放大后眼睛要跟着上移并外扩，否则会挤在脸中央显小
    _zb = r.Z(300)   # 与 build_head 相同的锚点（头底）
    head = dict(head,
                ax=head['ax'] * HEAD_SW, ay=head['ay'] * HEAD_SW,
                cx=head['cx'] * HEAD_SW)
    cz = _zb + (r.Z(212) - _zb) * HEAD_S
    ...
    eye_rx = r.L(68 / 2.0) * HEAD_SW
    eye_rz = r.L(87 / 2.0) * HEAD_SW
...
def build_feet(r: Refs, m_foot, coll):
    ...
    # 坑（第23轮，肉眼验收）：三趾各宽 51px 但趾心间距只有 31px ——
    # **相邻趾重叠 20px**，渲染出来糊成一整块，看不出是三根趾。
    # 参考正面 y582 左脚是清晰的三段 [67-86][91-128][132-182]
    # （宽 19/37/50，趾间留 4~5px 黑缝）。半径收到 17px 才能露出趾缝。
    # 17.4px 又收过头：脚变成三根细杆'香肠'，比糊成一团更糟 ——
    # 参考的脚是**饱满有肉**的，只是趾间有 4~5px 浅缝。取 22px 折中：
    # 趾心间距 31px、趾宽 44px，相邻只轻微相切，既有肉又能看出three趾。
    toe_r = r.L(22.0)      # 单根趾半径
            ...
            for (t, rf) in prof:
                ...
                for i in range(n):
                    ...
                    ring.append(bm.verts.new((
                        ...
                        y,
                        z0 + toe_r * 1.28 + toe_r * rf * 1.28 * math.sin(a))))
                rings.append(ring)
            for A, B in zip(rings, rings[1:]):
                for i in range(n):
                    j = (i + 1) % n
        ...
        for (t, rf) in pad_prof:
            ...
            for i in range(n):
                ...
                ring.append(bm.verts.new((
                    ...
                    y,
                    z0 + toe_r * 1.28 + toe_r * rf * 1.18 * math.sin(a))))
            rings.append(ring)
        for A, B in zip(rings, rings[1:]):
            for i in range(n):
                j = (i + 1) % n
...
def build_tail(r: Refs, m_wing, coll):
    ...
    # 第22轮（造型优先）：evaluated mesh 量出三片尾羽只占 X ±12px、
    # 全挤在中轴，且 Y 伸到 +0.626 远超身体后缘 +0.490 ——
    # 从**背面**看就是屁股正中一条竖直的「疤」，参考背面那里只有两道短横纹。
    # 参考侧面尾羽是贴着身体向后下方摊开的，不是戳在正后方。
    # 改：X 错开量拉大到 ±0.9（背面看散成两侧，不再叠成一条线），
    # lift 整体调低（尾羽下垂贴身），extend 收短避免超出身体后缘太多。
    for k, (xoff, lift, wide, extend) in enumerate((
            (-0.95, 0.10, 0.74, 0.74),    # 下片：几乎不翘、最短
            (0.00, 0.42, 0.92, 0.88),     # 中片
            (+0.88, 0.76, 0.80, 0.80))):  # 上片：略翘，拉开扇形
        bm = bmesh.new()
        rings = []
